...
public class LIS2 {
	...
	public static int LISLength(int [] arr){//O(nlogn)
		int size = arr.length;
		int d[] = new int[size];
		d[0] = arr[0];
		int end = 0;
	...
	public static int[] LIS22(int [] arr){//O(n^2*logn)
		int size = arr.length;
	...
	public static int[] LISCircle(int [] arr) {
		...
		for(int k = 0; k < size; k++) {
			...
			for(int i = 0; i < size; i++) {
				...
				int ind = binarySearchBetween(mat, end, arr[index]);
				for(int j = 0; j<ind; j++) {
					mat[ind][j] = mat[ind-1][j];
				}
				mat[ind][ind] = arr[index];
				if(ind>end){
					end++;
				}
				index++;
	...
	public static void main(String[] args) {
		int[]a1 = {2,-3,-4,90,-2,-1,-10,-9,-8};
		int[]a2  = {8,4,12,2,14,10,3,5,6,7};
		int[]a3  = {8,4,13,12};
		int[]a4 = {1,11,2,10,4,5,2,1};
		int[]a5 = {9,10,8,0,1,4,3,7};
	//	System.out.println(LISLength(a4));
		System.out.println(Arrays.toString(LIS2_ExhaustiveSearch(a1)));
//	System.out.println("lis(a1) = "+Arrays.toString(LIS22(a1)));
//	System.out.println("lis(a2) = "+Arrays.toString(LIS22(a2)));
//		System.out.println("lis(a3) = "+Arrays.toString(LIS22(a3)));
//	System.out.println("lisLen(a1) = "+LISLength(a1));
//	System.out.println("lisLen(a2) = "+LISLength(a2));
//	System.out.println("lisLen(a3) = "+LISLength(a3));	///lds
