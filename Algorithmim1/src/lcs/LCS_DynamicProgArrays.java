...
public class LCS_DynamicProgArrays {
		...
		for (i=1; i<row; i++) {
			for (j=1; j<col; j++) {
				if (X[i-1]==Y[j-1]){
					...
					System.out.print(Math.max(mat[i-1][j], mat[i][j-1]));
				}
			}
			System.out.println();
		}
...
// the function returns max common sequence of two integer sequences
	...
	public static void main(String[] args) {
		...
		int arr[] = {2,-3,4,90,-2,-1,-10,-9,-8};//{1,4,3,1,2,1}; //"bdcaba";
		int temp[] = {2,-3,4,90,-2,-1,-10,-9,-8};//{1,2,3,2,4,1,2}; //"abcbdab";
		Arrays.sort(temp);
	//	int[][]m = buldMatrix(temp, arr);

		//System.out.println("max seq len = "+maxSeqLength(X, Y));
		System.out.println(Arrays.toString(maxSequence(temp,arr)));
		//System.out.println(Arrays.toString(maxSequence(arr, temp)));
